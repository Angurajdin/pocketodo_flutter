...
class _NotificationPageDataState extends State<NotificationPageData>{
  ...
  Future<void> requestAction(Map<String, dynamic> data, int index, String action, List updatedNotfication) async{
    ...
    List requested = taskData.data()["requested"];
    requested.remove(data['emailid']);

    await FirebaseFirestore.instance.collection('tasks').doc(data['taskId']).update({
      "requested": requested
    });
    ...
    if(action=="accept"){
      ...
      await FirebaseFirestore.instance.collection('tasks').doc(data['taskId']).update({
        ...
        "membersPermission": [ "view", ...taskData.data()['membersPermission']]
      });
